// app/components/parent/GoogleAccountLinking.js
'use client';
...
export default function GoogleAccountLinking() {
  ...
  useEffect(() => {
    if (user?.uid) {
      ...
      console.log('👀 User provider data changed:', currentProviders);
      
      // Check if Google provider is present
      const hasGoogle = currentProviders.includes('google.com');
      console.log('🔍 Google provider present in user data:', hasGoogle);
      
      // Update local state if it differs from current
      if (hasGoogle !== isLinked) {
        console.log('🔄 Updating local state to match provider data:', hasGoogle);
  ...
  useEffect(() => {
    if (user === null) {
      console.log('👤 User signed out detected in GoogleAccountLinking');
    } else if (user?.uid) {
      console.log('👤 User signed in detected in GoogleAccountLinking:', user.uid);
    }
  }, [user]);
  ...
  const checkGoogleLinkStatus = async () => {
    try {
      if (!user?.uid) {
        console.log('⚠️ No user UID available for checking Google link status');
        return;
      }
      
      console.log('🔍 Checking Google link status for user:', user.uid);
      const linked = await hasGoogleLinked(user.uid);
      console.log('📊 Google link status result:', linked);
      ...
      if (currentUser) {
        ...
        console.log('🔍 Current user providers:', currentProviders);
        console.log('🔍 Has Google provider in current session:', currentProviders.includes('google.com'));
      }
  ...
  const handleGoogleUnlink = async () => {
    ...
    try {
      console.log('🔗 Attempting to unlink Google account for user:', user.uid);
      console.log('🔍 Current user provider data before unlink:', user.providerData?.map(p => p.providerId));
      ...
      if (result.success) {
        ...
        console.log('✅ Unlinking completed, refreshing status...');
        setTimeout(async () => {
          try {
            await checkGoogleLinkStatus();
            console.log('🔄 Link status refreshed after unlink');
          } catch (error) {
            console.log('ℹ️ Could not refresh link status, but unlinking was successful');
          }
        }, 1000);
      } else {
        console.error('❌ Unlink failed with result:', result);
        setMessage('Error unlinking Google account: ' + (result.error?.message || 'Unknown error'));
      }
    } catch (error) {
      console.error('❌ Error in handleGoogleUnlink:', error);
